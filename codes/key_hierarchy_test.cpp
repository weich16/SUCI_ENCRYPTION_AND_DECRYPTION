...
int main()
{
	...
	unsigned char ARFCN_DL[] = {0x00,0x0x,0x01};
	unsigned char EARFCN_DL[] = {0x00,0x0x,0x02};
	...
	HRES_star_generator(RES_star,HRES_star);
	cout << "HRES_star:";
	display(HRES_star,16);

	//33.501 A.6
	cout << "Test for 33.501 A.6\n\n";
	unsigned char Kseaf[16]; //or HXRES_star
	Kseaf_generator(Kausf,SNname,Kseaf);
	cout << "Kseaf:";
	display(HRES_star,32);
	...
	unsigned char Kng_RAN[32];
	Kng_RAN_new_gNB_generator(NH,PCI,EARFCN_DL,Kng_RAN);
	cout << "Kng_RAN_ng_eNB:";
	display(Kng_RAN,32);
	...
	cout << "Kasme_new:";
	display(Kamse_new,32);

	//33.501 A.15
	cout << "Test for 33.501 A.15\n\n";
	Kamf_new_from_ASME_generator(0x75,Kamse,NAS_link_COUNT,Kamf_new);
